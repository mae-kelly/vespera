...
impl OkxExecutor {
    pub async fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        Ok(OkxExecutor {
            client: Client::builder()
                .timeout(std::time::Duration::from_millis(100))  // 100ms timeout for speed
                .build()?,
            auth,
    ...
    async fn place_okx_take_profit_ladder(
        ...
        let tp_levels = vec![
            (entry_price * 0.985, total_size * 0.5),  // 50% at 1.5% profit
            (entry_price * 0.975, total_size * 0.3),  // 30% at 2.5% profit
            (entry_price * 0.965, total_size * 0.2),  // 20% at 3.5% profit
        ];
    ...
    async fn calculate_position_size(&self, entry_price: f64) -> Result<f64, Box<dyn std::error::Error>> {
        ...
        let position_percent = 0.02; // 2% risk per trade
