...
def setup_mandatory_gpu_acceleration():
    ...
    if system == "Darwin":
        ...
        print("=" * 50)
        try:
            gpu_info = subprocess.run(['sysctl', '-n', 'machdep.gpu.brand_string'],
                                    capture_output=True, text=True, timeout=5)
            if gpu_info.returncode == 0:
                gpu_brand = gpu_info.stdout.strip()
                print(f"gpu: {gpu_brand}")
                if any(chip in gpu_brand for chip in ['M1', 'M2', 'M3', 'M4']):
                    print(f"✅ Apple Silicon confirmed: {gpu_brand}")
                    if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
                        try:
                            ...
                            return {
                                ...
                                "device": "mps",
                                "optimized": True,
                                "gpu_info": {"name": gpu_brand, "apple_silicon": True},
                                "priority": 2
                            }
                            try:
