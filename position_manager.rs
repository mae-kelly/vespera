...
impl PositionManager {
    ...
    pub async fn add_position(&mut self, asset: &str, execution_result: &Value) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let tp_levels = vec![
            TpLevel { price: entry_price * 0.985, size: quantity * 0.5, filled: false },
            TpLevel { price: entry_price * 0.975, size: quantity * 0.3, filled: false },
            TpLevel { price: entry_price * 0.965, size: quantity * 0.2, filled: false },
        ];
        ...
        log::info!("Position added for {}: {} @ {:.2}", asset, quantity, entry_price);
    ...
    async fn check_take_profit_levels_static(position: &mut Trade, current_price: f64) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if tp_hit && !position.is_breakeven {
            ...
            if first_tp_filled {
                ...
                log::info!("Stop loss moved to breakeven for {}", position.asset);
            }
        }
        ...
        if position.side == "sell" && position.is_breakeven {
            ...
            if profit_distance > 0.0 {
                ...
                if trailing_stop < position.stop_loss {
                    ...
                    log::info!("Trailing stop updated for {}: {:.2}", position.asset, trailing_stop);
                }
            }
        }
