...
impl RiskEngine {
    ...
    pub async fn validate_trade_risk(
        ...
        if adjusted_confidence < 0.6 {
            return Ok(RiskCheck {
                approved: false,
                reason: format!("Volatility-adjusted confidence {:.2} too low", adjusted_confidence),
    ...
    pub async fn evaluate_positions(&mut self, positions: &[Value]) -> Result<(), Box<dyn std::error::Error>> {
        if positions.len() > self.max_open_positions as usize {
            log::warn!("Too many open positions: {} (max: {})", positions.len(), self.max_open_positions);
        }
        ...
        if self.session_pnl < -self.max_drawdown_percent {
            log::error!("Emergency: Session PnL {:.2}% exceeds maximum drawdown", self.session_pnl);
        }
        ...
        for position in positions {
            ...
            if entry_price > 0.0 {
                ...
                if pnl_percent < -5.0 {
                    ...
                    log::warn!("Large loss detected for {}: {:.2}%", asset, pnl_percent);
                }
            }
        }
    ...
    pub fn record_trade_result(&mut self, asset: &str, pnl: f64) {
        ...
        log::info!("Trade result recorded for {}: PnL {:.2}, Session PnL: {:.2}", 
