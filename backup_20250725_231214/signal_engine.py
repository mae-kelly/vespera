...
class PriceDataFeed:
    ...
    def _force_initialization(self):
        """Initialize with REST API data, then switch to WebSocket"""
        max_attempts = 5
        for attempt in range(max_attempts):
            try:
                ...
                # Initialize price history with simulated minute candles
    ...
    def _start_websocket_connection(self):
        """Start Coingecko WebSocket connection for real-time price data"""
        while self.running and self.reconnect_attempts < self.max_reconnect_attempts:
            try:
                ...
                def on_open(ws):
                    ...
                    # Subscribe to real-time price updates for BTC, ETH, SOL
                    subscribe_message = {
                        "type": "subscribe",
                        "channels": [
                ...
                if self.running and self.reconnect_attempts < self.max_reconnect_attempts:
                    ...
                    self._fallback_to_rest_polling()
                    break
    
    def _process_websocket_message(self, data):
        """Process real-time WebSocket price data from Coingecko"""
        try:
            if data.get("type") == "ticker":
                # Coingecko WebSocket ticker format
                ticker_data = data.get("data", {})
                coin_id = ticker_data.get("coin_id", "")
                
                # Map Coingecko coin IDs to our asset symbols
                asset_map = {
                    "bitcoin": "BTC",
                    "ethereum": "ETH", 
                    "solana": "SOL"
    ...
    def _fallback_to_rest_polling(self):
        ...
        while self.running:
            try:
                ...
                time.sleep(30)  # Poll every 30 seconds as fallback
                
            except Exception as e:
                logging.error(f"REST polling update failed: {e}")
                time.sleep(60)
...
def calculate_rsi_torch(prices: List[float], period: int = 14) -> float:
    """Calculate RSI using torch.nn.functional as required"""
...
def calculate_vwap(prices: List[float], volumes: List[float]) -> float:
    """Calculate VWAP using cupy_fallback as required"""
...
def calculate_price_change_cupy(prices: List[float], minutes: int = 60) -> float:
    """Calculate 1-hour percent change using cupy_fallback.diff over 60×1-min candles"""
    if len(prices) < minutes:
        raise Exception(f"Need {minutes} prices for change calc")
    
    prices_cp = cp.array(prices[-minutes:])
    # Use cupy_fallback.diff as required
...
def detect_volume_anomaly(volumes: List[float]) -> bool:
    """Volume anomaly detection: current_volume > mean_volume × 1.5"""
    if len(volumes) < 3:
        return False
...
def generate_signal(shared_data: Dict) -> Dict:
    """Generate trading signal with all required components"""
